"""
Script for generating the results of Section 6.1. Consistent data.
...
def circumcenter(N):
    ...
    r = mdl.addVar(vtype=gp.GRB.CONTINUOUS)
    ...
    for w in extreme_p:
        ...
        mdl.addConstr(gp.quicksum((w[i] - theta[i])**2 for i in range(n)) <= r)

    mdl.setParam('NonConvex', 2)
    mdl.addConstr(gp.quicksum(theta[i]**2 for i in range(n)) == 1)
